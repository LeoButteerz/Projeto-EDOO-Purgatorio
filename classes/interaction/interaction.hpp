#ifndef INTERACTION_HPP // evita definir novamente
...
#include <vector> // para a posição do mouse
    ...
    class Interactions // classe que abriga dados e métodos necessários para que ocorram interações após utilizar o mouse
    {
        private:
            ...
            std::vector<int> static mpos; // guarda a posição do mouse. como precisaríamos disso a todo momento no jogo completo, esse vetor é static

            void const position(std::vector<int>& pointer); // função que obtém a posição do mouse atual
        ...
        public:
            ...
            static std::vector<int>* mpubpos; // inicializado em 'interaction.hpp', aponta para mpos, que é private
